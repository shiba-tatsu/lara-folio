<template>
	<div class="works">
		<h5 class="title">&#10025;制作物&#10025;</h5>
		<div class="worksList">
			<div class="worksList__left">
				<a href="http://laravel-freesell.com">
					<img src="https://laraheroherotest.s3-ap-northeast-1.amazonaws.com/ZQcblx8Tfr3V7KoX75pVSWgVJvtr93WgC5QlDwAZ.png">
				</a>
			</div>
			<div class="worksList__right">
				<h5 class="worksList__right__title">freesell</h5>
				<p class="worksList__right__text">
					Laravelで初めて開発した個人制作物です。<br>
					フロントエンドの技術としてはVue.js,Bootstrapを中心に扱っています。<br>
					リアルタイムチャット機能やクレジットカード決済機能など、様々な機能を実装しています。<br>
					<br>
					URL: <a href="http://laravel-freesell.com">http://laravel-freesell.com</a><br>
					GitHub: <a href="https://github.com/shiba-tatsu/freesell">https://github.com/shiba-tatsu/freesell</a>
				</p>
			</div>
		</div>
		<div class="worksList">
			<div class="worksList__left">
				<a href="http://54.168.5.69">
					<img src="https://laraheroherotest.s3-ap-northeast-1.amazonaws.com/2HfUsKf1ziAO4w1H9vDfxm1SwpeBz2pvCqEqRsQW.png">
				</a>
			</div>
			<div class="worksList__right">
				<h5 class="worksList__right__title">larafolio</h5>
				<p class="worksList__right__text">
					このポートフォリオサイトになります。<br>
					Laravelを中心に開発を行い、Vue Routerを用いてSPAにし、スタイリングはScssを用いています。<br>
					<br>
					URL: <a href="http://54.168.5.69">http://54.168.5.69</a><br>
					GitHub: <a href="https://github.com/shiba-tatsu/lara-folio">https://github.com/shiba-tatsu/lara-folio</a>
				</p>
			</div>
		</div>
	</div>
</template>

<style lang="scss">

</style>